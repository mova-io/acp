...
Drive content authorization checks the fresh ADC account against the complete frozen discovered account/item identity. SharePoint acceptance freezes its configured library and an identity digest of tenant/client IDs; library/app changes and out-of-library scope fail before obtaining an app token. Future schedule edits do not alter accepted scope or policy. Global reset removes occurrence context and jobs through existing erasure tables. Tenant reset directly erases normalized-owner acceptance, pre-root elected jobs and accepted descendants in the same transaction, plus scheduled stage/input lineage, notifications and sweep result; it preserves live schedule configuration and another owner’s execution.
