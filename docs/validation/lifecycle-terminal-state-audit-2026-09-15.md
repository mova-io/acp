...
New listings replay exact source state, including safely identified legacy terminal snapshots. Tombstones survive delta absence/reappearance, ordinary scan deletion and history pruning. Explicit full administrative or tenant reset atomically erases the corresponding lifecycle ledger and source snapshots, including normalized tenant aliases, without restoring or recreating provider files. A subsequent scan depends on fresh authoritative provider data. Restoration receipts shadow older snapshots so historical terminal rows cannot undo an intentional restoration. Missing stable account/drive identity remains protected within the known scan and is explicitly reported as unavailable cross-scan linkage; it is never inferred from the name or another account.

The read-only lookup batches at most 400 unique exact identities and returns retained states plus explicit unavailable input indices. Exact legacy terminal snapshots supply missing ledger keys without writes; recorded restoration states take precedence. Assessment overrides apply only to recommendations; actual terminal items require verified provider restoration before assessment.
...
Schema revision 55 adds one table and preserves existing replicas' columns. The schema checksum guard is updated. Source lifecycle state participates in the existing full-reset erasure contract.

Delta removal may mean loss of access rather than deletion. This audit therefore does not turn arbitrary removal events into a claim that a provider deleted the file. A known terminal item remains protected if it disappears and later reappears. Live customer source accuracy, provider credentials, checkout behavior and actual source operations remain unverified by the offline fixtures. Parent owns merge and deployment. The scheduled scanner pre-content gate is a separate dependent change; this PR alone does not establish that monolithic scheduled scans skip content analysis.
