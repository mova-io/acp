...
export default function ReportModeMenu({ label = 'Report', formats = [], disabled = false, disabledReason = null, className = 'reports-menu', inline = false, progress = null }) {
    ...
    <div className="reportmode" style={{ display: inline ? 'block' : 'inline-block' }}>
      {inline ? (
        <div role="group" aria-label={label} className="reportmode-inline" style={{ padding: '4px 8px' }}>
          <div style={{ fontWeight: 700, fontSize: 12.5 }}>{busy ? (progress || `Generating ${busyLabel}…`) : label}</div>
          {choices}
        </div>
      ) : (
        <details className={className}>
          <summary className="exportbtn" aria-disabled={disabled || undefined} title={disabled ? disabledReason || undefined : undefined}>
            {busy ? (progress || `Generating ${busyLabel}…`) : label}
          </summary>
      ...
      <span role="status" aria-live="polite" className="muted" style={{ fontSize: 12, marginLeft: 6 }}>
        {busy ? (progress || `Generating ${busyLabel}…`) : status?.ok ? status.text : ''}
      </span>
      {status && !status.ok && (
