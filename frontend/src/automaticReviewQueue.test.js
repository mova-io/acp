...
const row={id:'proposal',file:'a.docx',status:'pending',hasProposal:true,after:'Alternative text',proposals:[{proposed_value:'Alternative text',source:'AI',model:'vision',model_call_id:'call'}],_raw:{corrected_artifact:'none',proposal_digest:'digest-test',finding_count:1,proposal_snapshot_ids:['snapshot'],source_revision:'source',decision_version:0,automatic_approval:{run_id:'run',source_revision:'source',state:'queued'}}}
...
it('retains manual, failed, rejected, stale and unadmitted work requiring input',()=>{
 const rows=[{...row,id:'manual',hasProposal:false,after:'',proposals:[]},{...row,id:'failed',status:'verification_failed'},{...row,id:'stale',stale:true},{...row,id:'missing',_raw:{corrected_artifact:'none',proposal_digest:'digest-test',...row._raw,proposal_snapshot_ids:[]}},{...row,id:'other',_raw:{corrected_artifact:'none',proposal_digest:'digest-test',...row._raw,automatic_approval:{...row._raw.automatic_approval,run_id:'other'}}}]
...
it('keeps server-owned deferral reasons visible without claiming queued or verified work',()=>{
 const deferred={...row,_raw:{corrected_artifact:'none',proposal_digest:'digest-test',...row._raw,automatic_approval:{run_id:'run',source_revision:'source',state:'review_required',owner:'You',reason:'This change requires individual judgment'}}}
...
it('routes exact-bound jobless approvals to Blocked while retaining terminal and active outcomes',()=>{
 const blocked={...row,status:'approved',_raw:{corrected_artifact:'none',proposal_digest:'digest-test',...row._raw,automatic_approval:{run_id:'run',source_revision:'source',proposal_snapshot_ids:['snapshot'],state:'blocked',owner:'You',reason:'No active application or verification job is recorded.'}}}
 ...
 const stale={...blocked,_raw:{corrected_artifact:'none',proposal_digest:'digest-test',...blocked._raw,automatic_approval:{...blocked._raw.automatic_approval,proposal_snapshot_ids:['old']}}}
 expect(workflowCounts(automaticReviewQueue([stale],policy))).toMatchObject({blocked:0,'awaiting-validation':1})
 const active={...blocked,_raw:{corrected_artifact:'none',proposal_digest:'digest-test',...blocked._raw,automatic_approval:{...blocked._raw.automatic_approval,state:'queued'}}}
...
it('rejects responsibility from another scan or missing bound snapshot manifest',()=>{
 ...
 const r={...row,scanId:'scan',_raw:{corrected_artifact:'none',proposal_digest:'digest-test',...row._raw,scan_id:'scan',automatic_approval:marker}}
 expect(automaticReviewQueue([r],policy)[0].automaticDisposition).toBeNull()
 expect(automaticReviewQueue([{...r,_raw:{corrected_artifact:'none',proposal_digest:'digest-test',...r._raw,automatic_approval:{...marker,scan_id:'scan',proposal_snapshot_ids:undefined}}}],policy)[0].automaticDisposition).toBeNull()
})
