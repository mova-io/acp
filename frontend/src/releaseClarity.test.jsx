...
it('keeps a new-release permission failure out of saved-destination recovery', async () => {
  ...
  await click(button(c, 'Publish batch (1)'))
  expect(c.textContent).toMatch(/does not have permission|write access/i)
  ...
  expect(button(c.querySelector('.release-recovery'), 'Retry')).toBeUndefined()
  expect(button(c, 'Publish batch (1)').disabled).toBe(false)
})
it('does not expose or blindly retry a missing saved destination', async () => {
  ...
  expect(recovery.textContent).toMatch(/resource.*missing or inaccessible/i)
  expect(recovery.textContent).toContain('release_resource_unavailable')
