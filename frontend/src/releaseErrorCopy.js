...
const DESTINATION = /release_destination_not_found|destination_not_found|graph\.microsoft\.com\/v1\.0\/drives\/[^/]+\/items\//i
...
export function releaseErrorCopy(error, providerName = 'the destination') {
  ...
  if (status === 404 || MISSING.test(text)) return {
    message: DESTINATION.test(`${code} ${text}`)
      ? `The saved ${providerName} destination is unavailable. It may have moved or been deleted. Choose or restore the destination before publishing again.`
      : 'A resource required for this release is unavailable. Check the saved destination and release status before trying again.',
    retryable: false,
    diagnosticCode: DESTINATION.test(`${code} ${text}`) ? 'destination_not_found' : 'release_resource_unavailable',
  }
  if (status === 401 || AUTH.test(text)) return {
  ...
  if (status === 403 || ACCESS.test(text)) return {
