...
const draft = (id, extra = {}) => ({id, file:`${id}.docx`, rule_id:'1.1.1',status:'pending',after:'An image description',hasProposal:true,
  _raw:{corrected_artifact:'none',proposal_digest:'digest-test',proposal_snapshot_ids:['p'],source_revision:1,decision_version:1},...extra})
const mixed = [
  draft(1, {_raw:{corrected_artifact:'none',proposal_digest:'digest-test',finding_count:40}}),
...
it('does not call generic manual reasons unsupported or incomplete assigned work automatic', () => {
  const result=unresolvedWorkSummary([draft(1,{_raw:{corrected_artifact:'none',proposal_digest:'digest-test',}}),draft(2,{automaticDisposition:{responsibility:'human',reason:'Manual work or no supported proposal writer'}})],{1:{state:'assigned'}})
