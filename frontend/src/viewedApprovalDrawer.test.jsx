...
const rowA = { id: 'drawer-a', scan_id: SCAN, file: FILE, rule_id: '2.4.4', rule_name: 'Link purpose', status: 'pending',
  decision_version: 2, source_revision: 'rev-A', proposal_snapshot_ids: ['snap-A1'], finding_count: 1, corrected_artifact: SHA_A, proposal_digest: DIG_A,
...
describe('FileDrawer — in-place approvals carry the VIEWED version', () => {
  ...
  it('F5: the proposal was refreshed in place while open: the VIEWED digest is sent and the 409 stated, no retry', async () => {
    const srv = server(rowA)
    const c = await openCard(srv)
  ...
  it('a HELD approval (approval_recheck_required) is offered in place as "Review and approve again", and re-approving sends the full current binding once', async () => {
    ...
    expect(toggle.textContent).toContain('approved earlier · version needs confirmation')
    await click(toggle)
    await settle()
    expect(c.querySelector('.drawer-reapprove')?.textContent).toContain('Approved earlier · version needs confirmation')
    expect(c.textContent).not.toMatch(/changed since|then changed/)
    await click(approveButton(c))
    await settle(120)
    expect(srv.puts).toHaveLength(1)
    expect(srv.puts[0].body).toMatchObject({ status: 'approved', approval_scope: 'single', expected_version: 2,
