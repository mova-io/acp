...
@pytest.mark.parametrize("trim", range(rr.SUMMARY_TRIM_LEVELS))
def test_a_closing_bold_decision_line_survives_the_summary_footer_strip(trim):
    """The summary strips trailing prose (the model's own footer). It used to strip the scan
    report's LAST section too — "N newly reported since the previous assessment" — at every trim
    level, so no scan Summary ever printed its comparison (PDF QA, 2026-09-18). Bold stays; the
    plain notes after it still go."""
    blocks = [{"k": "heading", "text": "Decision summary"},
              {"k": "callout", "text": "Outstanding: 4 open findings."},
              {"k": "heading", "text": "Since the previous assessment"},
              {"k": "text", "text": "Across the documents compared: 1 newly reported finding(s).", "o": {"bold": True}},
              {"k": "text", "text": "Documents: 4 compared.", "o": {"size": 9}},
              {"k": "text", "text": "Generated by ACP.", "o": {}}]
    kept, _ = rr.summary_blocks(blocks, trim)
    texts = [b.get("text") for b in kept]
    assert "Since the previous assessment" in texts
    assert "Across the documents compared: 1 newly reported finding(s)." in texts
    assert "Generated by ACP." not in texts and "Documents: 4 compared." not in texts


def test_a_document_cell_links_to_its_evidence_view_only_through_the_trusted_origin():
    """scanReport.docCell: a master-index document name opens that document in ACP — absolute
    against the trusted origin, the bare name without one, and never a link to another host."""
    href = "/?view=evidence&scan=s1&file=Board%2FQ1+pack.docx"
    blocks = [{"k": "heading", "text": "Documents"},
              {"k": "table", "caption": "Master index", "headers": ["Document", "State"],
               "rows": [[{"text": "Board/Q1 pack.docx", "href": href}, "Assessed"],
                        [{"text": "elsewhere.docx", "href": "https://evil.example/?view=evidence"}, "Assessed"],
                        ["plain.docx", "Assessed"]]}]
    body = rr.validate_request({"kind": "scan", "file": None, "mode": "full", "factsDigest": "a" * 64,
                                "model": model(blocks, kind="scan")})["model"]
    linked = rr.render_html(body, identity(), "full", "https://acp.example.com")
    assert f'<a href="https://acp.example.com{escape(href, quote=True)}">Board/Q1 pack.docx</a>' in linked
    assert 'href="https://evil.example' not in linked
    assert "<td>elsewhere.docx</td>" in linked
    unlinked = rr.render_html(body, identity(), "full", None)
    assert "<td>Board/Q1 pack.docx</td>" in unlinked and "view=evidence" not in unlinked
